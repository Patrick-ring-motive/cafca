...
async function produce({ topic, key, value }) {
  ...
  const metaKey = `${topic}:p${partition}:meta`;
  ...
  // Store message
  const messageKey = `${topic}:p${partition}:${offset}`;
...
async function consume({ topic, group = 'default', partition = 0, maxMessages = 10 }) {
  const cache = await getCache();
  const offsetKey = `consumer:${group}:${topic}:p${partition}`;
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (attempts === maxAttempts) {
          // Move to DLQ
          await cache.put(`dlq:${topic}:p${partition}:${msg.offset}`, new Response(JSON.stringify(msg)));
        }
